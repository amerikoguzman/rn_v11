...
<% 
/*------------------------------------------------------------------------------------------
Nombre de archivo: descarga_clave.jsp
Funcion: Descarga Clave a inventario de forma manual, se queda registro del asiento
  -----------------------------------------------------------------------------------------*/
...
int existe=0,am2_jv=0,exist=0,ap2_jv=0,total=0,pos=0,encontrado=0,encontrado_ing=0,exist_inv=0,ing_from_inv=0,sal_from_inv=0,exist_new=0,exist_sal=0,ban_sali=0,existencias=0,exist_ant=0;
...
// fin proceso Checar Existencias
	...
	if(but.equals("Salidas"))
	 ...
	 while(rset_001.next())
			 {
				 existencias=Integer.parseInt(rset_001.getString("existencias"));
				 existe=1;
				 //break;
			 }
	 ...
	 if( (existe==1) && (existencias!=0) && (existencias>=ap2_jv) )
	 {
	 ...
	 //proceso para decrementar las existencias
	 rset_001=stmt01.executeQuery("select clave,descrip,lote,caducidad,origen,id,existencias,ingreso,salidas from inventario_uni where clave='"+clave_jv+"' and lote= binary '"+lote_jv+"' and caducidad='"+cad_jv+"' and origen='"+origen_jv+"'; ");
	 while(rset_001.next())
			 {//inicio while
			  exist_ant=Integer.parseInt(rset_001.getString("existencias"));//lo que hay en existencias o cajas
			  ing_from_inv=Integer.parseInt(rset_001.getString("ingreso"));//lo que hay en ingreso
			  //ing_from_inv+=ap2_jv;
			  sal_from_inv=Integer.parseInt(rset_001.getString("salidas"));// toma exis en salidas
			  exist_new=sal_from_inv+ap2_jv;//lo que hay en salidas + lo que se va a descontar
			  exist_sal=ing_from_inv-exist_new;//existencias = entradas - sum(salidas)
			  // Se ingresan los datos para la tabla de entradas
		 stmt1.execute("insert into movimientos_salidas values ('"+rset_001.getString("clave")+"','"+rset_001.getString("descrip")+"','"+rset_001.getString("lote")+"','"+rset_001.getString("caducidad")+"','"+ap2_jv+"','1','"+ap2_jv+"','"+origen_jv+"','A','"+df.format(new java.util.Date())+"','"+hora_com+"','2')"); 
					 
					 //Se hace el update para la tabla inventario_uni
		 stmt1.execute("update inventario_uni set existencias='"+exist_sal+"',cajas='"+exist_sal+"',salidas='"+exist_new+"' where id='"+rset_001.getString("id")+"'");	
	//fin rutina de update 
	
	// tabla modificacion
	stmt1.execute("insert into modificacion values ('"+clave_jv+"','"+descrip_jv+"','"+rset_001.getString("lote")+"','"+rset_001.getString("caducidad")+"','"+exist_ant+"','"+origen_jv+"','"+ap2_jv+"','"+exist_sal+"','"+df.format(new java.util.Date())+"','"+am22_jv+"','SALIDA AJUSTE MANUAL','"+razon+"',current_timestamp,'-','-','-','-','-','-',0)");	
	// fin tabla
			  encontrado_ing=1;
			  ban_sali=0;
	...
	//   stmt1.execute("insert into modificacion values ('"+clave_jv+"','"+descrip_jv+"','-','-','"+exist_jv+"','"+origen_jv+"','"+ap2_jv+"','"+total+"','"+fechanew+"','"+am22_jv+"','AJUSTE MANUAL','',current_timestamp,'-','-','-','-','-','-',0)");	   
	 ...
	 if(ban_sali==0)
	  {%>
	 <script>
	 alert('SE REALIZÓ EL AJUSTE DE SALIDAS');
	 ...
	 %>
	 <script>
	 alert('NO SE REALIZÓ EL AJUSTE DE SALIDA');
	 document.form.txtf_cant.value="";
	 </script>
	 <%
	 }
...
<table width="666" height="346" border="3" align="center" cellpadding="2" bgcolor="#FFFFFF">
  <tr>
    ...
    <td width="650"><form id="form" name="form" method="post" action="descarga_clave.jsp?usuario=<%=am22_jv%>">
      <table width="650" height="377" border="0" align="center" cellpadding="2">
        ...
        <tr>
          ...
          <td colspan="2" class="style5"><input name="txtf_lote" type="text" id="txtf_lote" onChange="mayApemm(this.form)" onKeyPress="return handleEnter(this, event)" value="<%=lote_jv%>" size="10" readonly />
          <%
		   if(pos>=1)
		   {
		%>
        <select name="slct_lote" id="slct_lote" class="bodyText" onChange="setLot1(this.form);dimePropiedades(this.form);" onkeypress="return handleEnter(this, event)">
        <option>-- Lote --</option>
        <%
	...
	%>
        ...
        <tr>
          ...
          <td colspan="2" class="style5"><input name="txtf_cad" type="text" id="txtf_cad" onChange="mayApemm(this.form)" onKeyPress="return handleEnter(this, event)" value="<%=cad_jv%>" size="10" readonly />
             <%
		   if(pos>=1)
		   {
		%>
      <select name="slct_cad" id="slct_cad" class="bodyText" onChange="setCad(this.form)" onkeypress="return handleEnter(this, event)">
  ...
  <tr>
        ...
        <tr>
          ...
          <td colspan="2"><div align="center">
            <input type="submit" name="Submit" value="Salidas" onClick="return verifica_ING(document.forms.form);" />&nbsp;&nbsp;<!--input type="submit" name="Submit" value="Limpiar Claves" /-->
		...
		<tr>
          ...
          <td colspan="2">Total Piezas&nbsp;:&nbsp;&nbsp;<span class="Estilo7"><%=exist_sal%> </span></td>
          <td>&nbsp;</td>
        </tr>
		<tr>
