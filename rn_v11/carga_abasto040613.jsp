<%@ page contentType="text/html; charset=iso-8859-1" language="java" import="java.sql.*" import="java.io.*" import="javax.swing.filechooser.FileSystemView" errorPage="" %>
<%
/* -----------------------------------------------------------------------------------------------------
...
if(but.equals("CARGAR ABASTO"))
  {
	...
	ruta=ruta.replace('\\','/'); 
	rset=stmt.executeQuery("select * from receta_carga_archivos where archivo='"+ruta+"'");
       while (rset.next()) 
         {
			 existe_abasto=1;
			 break;
	...
	<script>
		...
		window.opener = top ;
   		self.location='csv_abasto.jsp?archivo=<%=ruta%>&usuario=<%=usuario_jv%>';
	</script>
	<%
	}
...
if(but.equals("IR AL PASO 2"))
  { // inicio de if but
  uni_send=request.getParameter("slct_uni");
  %>
<script>
		alert("PASO No. 2: Ingrese el ID del Paciente o Lea el Codigo de Barras del Paciente");
		window.opener = top ;
   		self.location='repor_ok.jsp?unis=<%=uni_send%>&id_med=<%=id_med%>';
	</script>
	<%
    //out.print("Ingreso a Paso 2"+uni_send);
	//folio_gnk=request.getParameter("txtf_id1");
  }
%>
...
<body>
<form action="carga_abasto.jsp?id_med=<%=id_med%>&usuario=<%=usuario_jv%>" method="post" name="form">
...
<tr>
    <th colspan="4" scope="row" align="left"><hr /><a href="index.jsp" title="Haga clic para Regresar a Menu" accesskey="q">Regresar a Men&uacute;</a></th>
    </tr>
 <tr>
    <th colspan="4" scope="row"><hr /></th>
    </tr>
  <tr>
    <th width="142" scope="col"><img src="imagenes/usb2.jpg" width="82" height="67" /></th>
    <th colspan="2" scope="col"><span class="style1">CARGAR ARCHIVO CON EL ABASTO DE LA UNIDAD</span><br />
      <br />
      ESCOJA LA RUTA DONDE SE ENCUENTRE EL ARCHIVO *</th>
  ...
  <tr>
    <th scope="row">&nbsp;</th>
    <td width="180">Seleccione Archivo:</td>
    <td width="449" align="left"><input type="file" name="datasize" id="datasize" size="30"></td>
    <td>&nbsp;</td>
  </tr>
  <tr>
    <th scope="row">&nbsp;</th>
  ...
  <tr>
    ...
    <tr>
    <th colspan="4" scope="row"><img src="imagenes/nay_ima1.jpg" width="277" height="85" /><hr /></th>
    </tr>
</table>
</center>
</form>
