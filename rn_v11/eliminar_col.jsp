...
String cb_jv="",nombre_jv="",edad_jv="",folio_jv="",cause_jv="",foliore_jv="",foliore2_jv="",radiopro_jv="",radiosur_jv="",date_jv="",reloj_jv="",ela2_jv="",juris_jv="",clave_jv="",eliminar_jv="",usu_jv="",equipo_jv="",integrantes_jv="",recibe_id_jv="",lotes_jv="",cadus_jv="",part_jv="";
//String eliminar_jv="";
...
if(entrar==true)
{
	...
	self.location='eliminar_re_col_fecha.jsp?id_prov=<%=eliminar_jv%>&foliore=<%=foliore_jv%>&encar=<%=ela2_jv%>&juris=<%=no_jur%>&uni=<%=nom_unidad%>&juris1=<%=juris_jv%>&clave_uni=<%=clave_jv%>&descrip1=&cv_dgo=<%=cv_dgo_jv%>&cv_uni=<%=cv_uni_jv%>&cv_mpio=<%=cv_mpio_jv%>&day5=<%=t1_jv%>&mes5=<%=t2_jv%>&aa5=<%=t3_jv%>&servicio=<%=servicio_jv%>&encarser=<%=encarser_jv%>&clave1=<%=clave1_jv%>&sol1=<%=sol1_jv%>&sur1=<%=sur1_jv%>&lote=<%=lotes_jv%>&cadu=<%=cadus_jv%>&part=<%=part_jv%>&ban=<%=tipo_rec%>';
