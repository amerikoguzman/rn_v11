...
int existe=0,am2_jv=0,exist=0,ap2_jv=0,total=0,pos=0,encontrado=0,encontrado_ing=0,exist_inv=0,ing_from_inv=0,sal_from_inv=0,exist_new=0,exist_sal=0,ban_sali=0,existencias=0,exist_ant=0,cant_mov=0;
...
if(but.equals("Checar Existencias"))
 {// inicio de if
  ...
  origen_jv=request.getParameter("txtf_origen");
  	 ...
  	 rset_001=stmt01.executeQuery("select clave,lote,caducidad,origen,id,existencias from inventario_uni where clave='"+clave_jv+"' and lote='"+lote_jv+"' and caducidad='"+cad_jv+"' and origen='"+origen_jv+"'; ");
	 while(rset_001.next())
			 {//inicio while
...
// fin proceso Checar Existencias
	...
	if(but.equals("Salidas"))
	 ...
	 // proceso si existe la clave en el inventario
	   rset_001=stmt01.executeQuery("select clave,descrip,lote,caducidad,origen,id,existencias,ingreso,salidas,id from inventario_uni where clave='"+clave_jv+"' and lote='"+lote_jv+"' and caducidad='"+cad_jv+"' and origen='"+origen_jv+"'; ");
	 while(rset_001.next())
			 {
	...
	cant_mov=ap2_jv;
	cant_mov*=-1;
	stmt1.execute("insert into modificacion values ('"+rset_001.getString("clave")+"','"+rset_001.getString("descrip")+"','"+rset_001.getString("lote")+"','"+rset_001.getString("caducidad")+"','"+exist_ant+"','"+origen_jv+"','"+cant_mov+"','"+exist_sal+"','"+df.format(new java.util.Date())+"','"+am22_jv+"','SALIDA AJUSTE MANUAL','"+razon+"',current_timestamp,'-','-','4','-','-','"+exist_sal+"',0)");	
	// fin tabla
