...
/*----------------------------------------------------------------------------------------------------------------------
Nombre del Archivo: eliminar_re_fecha_mf.jsp
------------------------------------------------------------------------------------------------------------------------*/
...
stmt1 = conn_001.createStatement();
	...
	// fin de proceso -----------------------------------------------------------------				
		if(sol1_jv.equals(sur1_jv))
		{
			...
			stmt1.execute("insert into movimientos_salidas values ('"+clave1_jv+"','"+descrip1_jv+"','"+lotes_jv+"','"+cadus_jv+"','"+cant_sur_neg+"','1','"+cant_sur_neg+"','"+part_jv+"','-','"+df.format(new java.util.Date())+"','"+hora_com+"','3')"); 
			// fin agregar
			...
			while (rset_re.next())
			{
			 ...
			 stmt1.execute("update inventario_uni set salidas='"+var_salidas+"',cajas='"+var_exist+"', existencias='"+var_exist+"' where id='"+id_inv+"'");
			  ...
			  // se agrega info a la tabla modificacion
				      stmt1.execute("insert into modificacion values ('"+clave1_jv+"','"+descrip1_jv+"','"+lotes_jv+"','"+cadus_jv+"','"+cant_ant+"','"+part_jv+"','"+sur1_jv+"','"+var_exist+"','"+df.format(new java.util.Date())+"','"+t2_jv+"','ELIMINACION DE INSUMO','REINTEGRACION DE SALIDA DE INSUMO AL INVENTARIO',current_timestamp,'"+foliore_jv+"','-','10','-','"+present1_jv+"','"+var_exist+"',0)");						 
					 // fin tabla modi 		
		}			
