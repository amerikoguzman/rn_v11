...
/*----------------------------------------------------------------------------------------------------------------------
Nombre del Archivo: eliminar_re_col_fecha2.jsp
Fecha:2013
Autor: RHW
  ----------------------------------------------------------------------------------------------------------------------*/
String cb_jv="",nombre_jv="",edad_jv="",folio_jv="",cause_jv="",foliore_jv="",foliore2_jv="",radiopro_jv="",radiosur_jv="",date_jv="",reloj_jv="",ela2_jv="",juris_jv="",clave_jv="",eliminar_jv="",usu_jv="",equipo_jv="",integrantes_jv="",recibe_id_jv="",t1_jv="",t2_jv="",t3_jv="",tfecha_jv="",servicio_jv="",encarser_jv="",lotes_jv="",cadus_jv="",id_inv="",usuario="",cant_ant="";
int tipo_rec=0;
//String eliminar_jv="";
			...
			while (rset_re.next())
			{
			 ...
			 stmt1.execute("update inventario_uni set salidas='"+var_salidas+"',cajas='"+var_exist+"', existencias='"+var_exist+"' where id='"+id_inv+"'");	
			   ...
			   // se agrega info a la tabla modificacion
				      stmt1.execute("insert into modificacion values ('"+clave1_jv+"','"+descrip1_jv+"','"+lotes_jv+"','"+cadus_jv+"','"+cant_ant+"','"+part_jv+"','"+sur1_jv+"','"+var_exist+"','"+date3+"','"+usuario+"','ELIMINACION DE INSUMO','REINTEGRACION AL INVENTARIO VIA MODIFICACION DE RECETA EN COLECTIVA',current_timestamp,'"+foliore_jv+"','-','10','-','"+present1_jv+"','"+var_exist+"',0)");						 
					 // fin tabla modi 	
		}				
...
<body>
<form action="eliminar_re_col_fecha2.jsp?id_prov=<%=eliminar_jv%>&foliore=<%=foliore_jv%>&encar=<%=ela2_jv%>&juris=<%=no_jur%>&uni=<%=nom_unidad%>&juris1=<%=juris_jv%>&clave_uni=<%=clave_jv%>&clave1=&descrip1=&sur1=&sol1=&day5=<%=t1_jv%>&mes5=<%=t2_jv%>&aa5=<%=t3_jv%>&cv_dgo=<%=cv_dgo_jv%>&cv_uni=<%=cv_uni_jv%>&cv_mpio=<%=cv_mpio_jv%>&servicio=<%=servicio_jv%>&encarser=<%=encarser_jv%>&ban=<%=tipo_rec%>&usuario=<%=usuario%>" method="post" name="form" onSubmit="javascript:return ValidateFar1(this)">
...
self.location='rc_modi2.jsp?id_prov=<%=eliminar_jv%>&foliore=<%=foliore_jv%>&encar=<%=ela2_jv%>&juris=<%=no_jur%>&uni=<%=nom_unidad%>&juris1=<%=juris_jv%>&clave_uni=<%=clave_jv%>&clave1=&descrip1=&sur1=&sol1=&part=&cv_dgo=<%=cv_dgo_jv%>&cv_uni=<%=cv_uni_jv%>&cv_mpio=<%=cv_mpio_jv%>&day5=<%=t1_jv%>&mes5=<%=t2_jv%>&aa5=<%=t3_jv%>&servicio=<%=servicio_jv%>&encarser=<%=encarser_jv%>&ban=<%=tipo_rec%>&usuario=<%=usuario%>'
</script>
<%}else{%><script>
self.location='rc_modi2.jsp?id_prov=<%=eliminar_jv%>&foliore=<%=foliore_jv%>&encar=<%=ela2_jv%>&juris=<%=no_jur%>&uni=<%=nom_unidad%>&juris1=<%=juris_jv%>&clave_uni=<%=clave_jv%>&clave1=&descrip1=&sur1=&sol1=&part=&cv_dgo=<%=cv_dgo_jv%>&cv_uni=<%=cv_uni_jv%>&cv_mpio=<%=cv_mpio_jv%>&day5=<%=t1_jv%>&mes5=<%=t2_jv%>&aa5=<%=t3_jv%>&servicio=<%=servicio_jv%>&encarser=<%=encarser_jv%>&ban=<%=tipo_rec%>&usuario=<%=usuario%>'
</script>
