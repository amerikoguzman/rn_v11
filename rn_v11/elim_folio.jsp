...
//out.print(url);
qry_receta_jv="select id, cant_sur, clave, descrip, lote, caducidad, partida from receta where folio_re='"+folio_jv+"'";
//out.println(qry_receta_jv);
if(!folio_jv.equals("") || folio_jv.equals("")){
ResultSet rset =null;
...
while(rset.next())
{
	   ...
	   rset_re = stmt_re.executeQuery("select cajas,existencias,salidas,id from inventario_uni where clave='"+clave1_jv+"' and lote='"+lotes_jv+"' and caducidad='"+cadus_jv+"' and origen='"+part_jv+"'; ");
			...
			qry_mod="insert into modificacion values ('"+clave1_jv+"','"+descrip1_jv+"','"+lotes_jv+"','"+cadus_jv+"','"+cant_ant+"','"+part_jv+"','"+cant_sur_jv+"','"+var_exist+"','"+df.format(new java.util.Date())+"','"+usuario+"','ELIMINACION DE INSUMO','REINTEGRACION DE SALIDA DE INSUMO AL INVENTARIO',current_timestamp,'"+folio_jv+"','-','10','-','-','"+var_exist+"',0);";
			stmt1.execute(qry_mod);
			
			qry_ins_elim="insert into receta_elim (select * from receta where id = '"+id_inv_jv+"');";
			//out.println("<br>"+qry_ins_elim);
			stmt1.execute(qry_ins_elim);
		}
		qry_elim="delete from receta where folio_re='"+folio_jv+"'";
