...
<% 
/* ----------------------------------------------------------------------------------------------------
Nombre de JSP: receta_por_surtir.jsp
Función      : Muestra aquellos folios pendientes por surtir y surte la receta en caso de que haya y halle
               existencias
 ---------------------------------------------------------------------------------------------------- */
...
//proceso para obtener los folios con el estatus PS
	rset2= stmt2.executeQuery("select folio_re,nombre_pac from receta where status_receta='PENDIENTE POR SURTIR';");
	 while(rset2.next())
	  {
...
if(but.equals("Por Folio"))
	 ...
	 rset2= stmt2.executeQuery("select * from receta where folio_re='"+foliore_jv+"' and status_receta='PENDIENTE POR SURTIR' ");
	  while(rset2.next())
	  {
	  	cont1++;
	  }
...
if(but.equals("Imprimir"))
     {
	  ...
	  %>
	  <script>
	  self.location='ticket_surt.jsp?folio=<%=foliore2_jv%>&receta5=RF&encar=<%=encar_jv%>&uni=<%=nom_unidad%>&juris=<%=no_jur%>&juris1=<%=juris_jv%>&clave_uni=<%=clave_jv%>&cv_dgo=<%=cv_dgo_jv%>&cv_uni=<%=cv_uni_jv%>&cv_mpio=<%=cv_mpio_jv%>&part=<%=part_jv%>&id_med=<%=id_med_jv%>';
        ...
        <tr>
		 ...
		 %>
          ...
          <td width="250" align="center" class="style2">
            FECHA:
                  <input name="txtf_t1" type="text" id='txtf_t1' onKeyPress="return handleEnter(this, event)" onKeyUp="putDays()" value="<%= day1 %>/<%=res%>/<%= year %>" size="10" class="style13"/>&nbsp;&nbsp;<a href="reimp.jsp?uni=<%=nom_unidad%>&juris=<%=no_jur%>&nombre=<%=nombre_jv%>&edad=<%=edad_jv%>&folio=<%=folio_jv%>&clave1=<%=clave1_jv%>&descrip1=<%=descrip1_jv%>&present1=<%=present1_jv%>&indica1=<%=indica1_jv%>&sol1=<%=sol1_jv%>&sur1=<%=sur1_jv%>&cause=<%=cause_jv%>&foliore=<%=foliore_jv%>&encar=<%=encar_jv%>&juris1=<%=juris_jv%>&clave_uni=<%=clave_jv%>&univer=<%=uni_jv%>&cedu=<%=cedu_jv%>&nomed=<%=nomed_jv%>&cv_dgo=<%=cv_dgo_jv%>&cv_uni=<%=cv_uni_jv%>&cv_mpio=<%=cv_mpio_jv%>&day5=<%=t1_jv%>&mes5=<%=t2_jv%>&aa5=<%=t3_jv%>&programa=<%=programa_jv%>&carnet=-&part=<%=part_jv%>" target="_blank">Reimprir TICKET</a>
          </td>
        </tr>
    </table>    </td>
  </tr>
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;APELLIDO PATERNO </span><span class="style2">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;APELLIDO MATERNO&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;NOMBRE&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span></label></td>
            ...
            <tr>
              ...
              CAUSES:
              <input name="txtf_cause" type="text" class="style13" onKeyPress="return handleEnter(this, event)" value="<%=cause_jv%>" size="8" onClick="putEmpty()" /><br /><br /><a href="index_carga_clave.jsp" target="_blank"><strong>Agregar Clave al Inventario</strong></a>| <a href="existencias.jsp" target="_blank">Ver Existencias</a>
              </span>
              </td>
              <td>&nbsp;</td>
            </tr>
  ...
  %>
      ...
      <%
	  	if(c_s==0)
			{
	  %>
              <td class="style11"><input  type="button" id="btn_<%=eliminar_jv%>" name="btn_<%=eliminar_jv%>" class="subHeader" value="Surtir" onclick='location.href="surtir.jsp?id_prov=<%=eliminar_jv%>&uni=<%=nom_unidad%>&juris=<%=no_jur%>&nombre=<%=nombre_jv%>&edad=<%=edad_jv%>&folio=<%=folio_jv%>&clave1=<%=clave1_jv%>&descrip1=<%=descrip1_jv%>&present1=<%=present1_jv%>&indica1=<%=indica1_jv%>&sol1=<%=sol1_jv%>&sur1=<%=sur1_jv%>&clave2=<%=hora_ini_jv%>&descrip2=<%=fuente%>&present2=<%=present2_jv%>&indica2=<%=indica2_jv%>&sol2=<%=afiliado_jv%>&sur2=<%=sur2_jv%>&clave3=<%=clave3_jv%>&descrip3=<%=descrip3_jv%>&present3=<%=present3_jv%>&indica3=<%=indica3_jv%>&sol3=<%=sol3_jv%>&sur3=<%=sur3_jv%>&cause=<%=cause_jv%>&foliore=<%=foliore_jv%>&encar=<%=encar_jv%>&juris1=<%=juris_jv%>&clave_uni=<%=clave_jv%>&univer=<%=uni_jv%>&cedu=<%=cedu_jv%>&nomed=<%=nomed_jv%>&cv_dgo=<%=cv_dgo_jv%>&cv_uni=<%=cv_uni_jv%>&cv_mpio=<%=cv_mpio_jv%>&part=<%=part_jv%>&id_med=<%=id_med_jv%>&day5=<%=t1_jv%>&carnet=<%=carnet_jv%>&day5=<%=t1_jv%>&mes5=<%=t2_jv%>&aa5=<%=t3_jv%>&fecha=<%=fechanew%>&afi=<%=afiliado_jv%>&fuente=<%=fuente%>"'/>&nbsp;</td>
      <%
		}// fin if c_s
	  %>
            </tr>
