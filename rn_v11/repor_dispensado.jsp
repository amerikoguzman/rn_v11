...
<p>
  <%
	...
	if (date1_jv==null && date2_jv==null){
 	rset__001=stmt__001.executeQuery("select clave, descrip, sum(cant_sur) from receta where cant_sur!=0 group by clave order by (clave+0)");
	rset__002=stmt__002.executeQuery("select sum(cant_sur) from receta");
	 }
	 else {
	 rset__001=stmt__001.executeQuery("select clave, descrip, sum(cant_sur) from receta where fecha_re between '"+date1_jv+"' and '"+date2_jv+"' and cant_sur!=0 group by clave order by (clave+0)");
	rset__002=stmt__002.executeQuery("select sum(cant_sur) from receta where fecha_re between '"+date1_jv+"' and '"+date2_jv+"' and cant_sur!=0 ");
 	}
