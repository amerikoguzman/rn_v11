...
String t2_jv=request.getParameter("f2");
	//out.print(t2_jv);
 rset = stmt.executeQuery("SELECT STR_TO_DATE('"+t1_jv+"', '%d/%m/%Y')"); //;SELECT STR_TO_DATE('"+date_jv+"', '%m/%d/%Y')
                    while(rset.next())
					{
                    	fecha1= rset.getString("STR_TO_DATE('"+t1_jv+"', '%d/%m/%Y')");//STR_TO_DATE('"+date_jv+"', '%m/%d/%Y')
						//out.print(fecha2);
					 }

 rset2 = stmt2.executeQuery("SELECT STR_TO_DATE('"+t2_jv+"', '%d/%m/%Y')"); //;SELECT STR_TO_DATE('"+date_jv+"', '%m/%d/%Y')
                    while(rset2.next())
					{
                    	fecha2= rset2.getString("STR_TO_DATE('"+t2_jv+"', '%d/%m/%Y')");//STR_TO_DATE('"+date_jv+"', '%m/%d/%Y')
						//out.print(t2_jv);
						//out.print(fecha2);
					 }
					 //out.print("Hola");
...
<form action="rep_nivserv.jsp" method="get">
  Seleccione las fechas de la consulta. <a href="gnr_rep_nivserv.jsp?f1=<%=fecha1%>&f2=<%=fecha2%>"><img src="imagenes/exc.jpg" width="40" height="40" /></a><br />
  Del :
