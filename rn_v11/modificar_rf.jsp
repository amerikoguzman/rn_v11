...
<%
/* ----------------------------------------------------------------------------------
Nombre de Archivo: modificar_rf.jsp
Función: Modifica datos de la receta
-------------------------------------------------------------------------------------*/
// Conexiones a la BDD
...
String hora_ini_jv="";
String lugar_jv="";
String cb_jv="",nombre_jv="",edad_jv="",folio_jv="",cause_jv="",foliore_jv="",foliore2_jv="",radiopro_jv="",radiosur_jv="",date_jv="",reloj_jv="",encar_jv="",juris_jv="",clave_jv="",eliminar_jv="",usu_jv="",equipo_jv="",integrantes_jv="",recibe_id_jv="",t2_jv="",t3_jv="",id_send="";
String but="r";
String but2="r";
String nom_unidad="",no_jur="",cant_ant="";
		...
		rset_re = stmt_re.executeQuery("select cajas,existencias,salidas,id,ingreso from inventario_uni where clave='"+clave1_jv+"' and lote='"+lote+"' and caducidad='"+caducidad+"' and origen='"+part_jv+"'; ");
			...
			//sal_idas=sal_inv-reint_dif
			 ...
			 // se agrega info a la tabla modificacion
				      stmt.execute("insert into modificacion values ('"+clave1_jv+"','"+descrip1_jv+"','"+lote+"','"+caducidad+"','"+cant_ant+"','"+part_jv+"','"+reint_dif+"','"+existencias+"','"+df.format(new java.util.Date())+"','"+t2_jv+"','MODIFICACION DE INSUMO','SALIDA DE INSUMO AL INVENTARIO VIA MODIFICACION MANUAL',current_timestamp,'"+foliore_jv+"','-','11','-','-','"+existencias+"',0)");						 
					 // fin tabla modi 		
			
		stmt.execute("insert into receta_modi(SELECT * from receta where folio_re='"+foliore_jv+"'); ");	
		stmt.execute("update receta set cant_sol='"+cant_mod_sol+"',cant_sur='"+cant_mod_sur+"' where id='"+id_send+"'");
...
<form name="form" method="post" action="modificar_rf.jsp?uni=<%=nom_unidad%>&juris=<%=no_jur%>&nombre=<%=nombre_jv%>&edad=<%=edad_jv%>&folio=<%=folio_jv%>&clave1=<%=clave1_jv%>&descrip1=<%=descrip1_jv%>&present1=<%=present1_jv%>&indica1=<%=indica1_jv%>&sol1=<%=sol1_jv%>&sur1=<%=sur1_jv%>&clave2=<%=clave2_jv%>&descrip2=<%=descrip2_jv%>&present2=<%=present2_jv%>&indica2=<%=indica2_jv%>&sol2=<%=sol2_jv%>&sur2=<%=sur2_jv%>&clave3=<%=clave3_jv%>&descrip3=<%=descrip3_jv%>&present3=<%=present3_jv%>&indica3=<%=indica3_jv%>&sol3=<%=sol3_jv%>&sur3=<%=sur3_jv%>&cause=<%=cause_jv%>&foliore=<%=foliore_jv%>&encar=<%=encar_jv%>&juris1=<%=juris_jv%>&clave_uni=<%=clave_jv%>&univer=<%=uni_jv%>&cedu=<%=cedu_jv%>&nomed=<%=nomed_jv%>&id_prov=<%=eliminar_jv%>&cv_dgo=<%=cv_dgo_jv%>&cv_uni=<%=cv_uni_jv%>&cv_mpio=<%=cv_mpio_jv%>&part=<%=part_jv%>&id_med=<%=id_med_jv%>&day5=<%=t1_jv%>&mes5=<%=t2_jv%>&aa5=<%=t3_jv%>&carnet=<%=carnet_jv%>&programa=<%=programa_jv%>&perfil=aleman&sexo=<%=sexo_jv%>&lote=<%=lote%>&cadu=<%=caducidad%>&id=<%=id_send%>">
        ...
        <tr>
          <td height="123" colspan="3" class="style4"><table width="863" border="0" align="center" cellpadding="2">
            ...
            <tr>
              ...
              &nbsp; <a href="modi2_receta.jsp?uni=<%=nom_unidad%>&juris=<%=no_jur%>&nombre=<%=nombre_jv%>&edad=<%=edad_jv%>&folio=<%=folio_jv%>&clave1=<%=clave1_jv%>&descrip1=<%=descrip1_jv%>&present1=<%=present1_jv%>&indica1=<%=indica1_jv%>&sol1=<%=sol1_jv%>&sur1=<%=sur1_jv%>&clave2=<%=clave2_jv%>&descrip2=<%=descrip2_jv%>&present2=<%=present2_jv%>&indica2=<%=indica2_jv%>&sol2=<%=sol2_jv%>&sur2=<%=sur2_jv%>&clave3=<%=clave3_jv%>&descrip3=<%=descrip3_jv%>&present3=<%=present3_jv%>&indica3=<%=indica3_jv%>&sol3=<%=sol3_jv%>&sur3=<%=sur3_jv%>&cause=<%=cause_jv%>&foliore=<%=foliore_jv%>&encar=<%=encar_jv%>&juris1=<%=juris_jv%>&clave_uni=<%=clave_jv%>&univer=<%=uni_jv%>&cedu=<%=cedu_jv%>&nomed=<%=nomed_jv%>&id_prov=<%=eliminar_jv%>&cv_dgo=<%=cv_dgo_jv%>&cv_uni=<%=cv_uni_jv%>&cv_mpio=<%=cv_mpio_jv%>&part=<%=part_jv%>&id_med=<%=id_med_jv%>&day5=<%=t1_jv%>&mes5=<%=t2_jv%>&aa5=<%=t3_jv%>&carnet=<%=carnet_jv%>&programa=<%=programa_jv%>&perfil=aleman&sexo=<%=sexo_jv%>&lote=<%=lote%>&cadu=<%=caducidad%>&id=<%=id_send%>">Regresar</a></td>
            </tr>
