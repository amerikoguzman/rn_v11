...
<% 
/* ----------------------------------------------------------------------------------------------------
Nombre de JSP: receta_col_checar.jsp
Función      : Checa RC, imprime ticket
 ---------------------------------------------------------------------------------------------------- */
// Conexión a la BDD
...
//proceso para obtener los folios con el estatus PS
	rset2= stmt1.executeQuery("select folio_re,nombre_pac from receta_colectiva group by folio_re order by folio_re+0;");
	 while(rset2.next())
	  {
	 ...
	 rset1= stmt.executeQuery("select * from receta_colectiva where folio_re='"+foliore_jv+"' ");
	  while(rset1.next())
                    ...
                    while(rset_date.next())
					{
					 ...
					 ela2_jv=rset1.getString("encargado");
...
if(but.equals("Imprimir"))
     {
	  ...
	  %>
	  <script>
	  self.location='ticket2_sur_checar.jsp?folio=<%=foliore2_jv%>&folio2=<%=customerIds%>&receta5=RC&encar=<%=ela2_jv%>&uni=<%=nom_unidad%>&juris=<%=no_jur%>&juris1=<%=juris_jv%>&clave_uni=<%=clave_jv%>&cv_dgo=<%=cv_dgo_jv%>&cv_uni=<%=cv_uni_jv%>&cv_mpio=<%=cv_mpio_jv%>&encarser=<%=encarser_jv%>';
	  </script>
	  <%
}	
// fin Imprimir
...
<form name="form" method="post" action="receta_col_checar.jsp?uni=<%=nom_unidad%>&juris=<%=no_jur%>&nombre=<%=nombre_jv%>&edad=<%=edad_jv%>&folio=<%=folio_jv%>&clave1=<%=clave1_jv%>&descrip1=<%=descrip1_jv%>&present1=<%=present1_jv%>&indica1=<%=indica1_jv%>&sol1=<%=sol1_jv%>&sur1=<%=sur1_jv%>&cause=<%=cause_jv%>&encar=<%=ela2_jv%>&juris1=<%=juris_jv%>&clave_uni=<%=clave_jv%>&cv_dgo=<%=cv_dgo_jv%>&cv_uni=<%=cv_uni_jv%>&cv_mpio=<%=cv_mpio_jv%>&day5=<%=t1_jv%>&mes5=<%=t2_jv%>&aa5=<%=t3_jv%>&servicio=<%=servicio_jv%>&part=<%=part_jv%>&foliore=<%=foliore_jv%>&encarser=<%=encarser_jv%>">
      ...
      <table width="912" height="61" border="0" align="center" cellpadding="2">
        <tr>
		 ...
		 %>
          ...
          <label>          </label></td>
          <td width="173"><span class="style2">HORA:</span>            <input name="reloj" type="text"  class="Estilo5" onKeyPress="return handleEnter(this, event)" value="<%=hora_rec%>" size="15" readonly ></td>
          <td width="276" align="left" class="style2">FECHA
           
                <input type='text' name='txtf_t3' id='txtf_t3' title='DD/MM/AAAA' value="<%=t3_jv%>" size='10' maxlength='10' onclick='scwShow(this, event);' readonly class="Estilo5"/>
       </td>
        </tr>
    </table>    </td>
  </tr>
