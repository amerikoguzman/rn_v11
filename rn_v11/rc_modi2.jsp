...
String financiamiento[]= new String[50000];
String folios[]= new String[50000];
String origen[]= new String[50000];
...
int pos=0,tam2=0,x1=0,cont3=0,ban=0,posf=0,ban2=0,cant2_jv=0,cv_med2=0,cv_med3=0,cont1=0,cont5=0,cont6=0,sur2=0,sol2=0,mtotal=0,total2=0,total=0,flag=0,customerIds=0,cant_pasti_ampu=0,cant_surpas_amp=0,ttcant_surpas_amp=0,ttcant_surpas_amp2=0,ban_clave=0,tipo_rec=0,no_existe=0,surtida=0,cant_x_sutir_neg=0;;
String encontrado="",cant_jv="",financiamiento1="",cv_finan_jv="",layout="",status="",cant_pasti="",cant_surpas="",count_cv="",servicio_jv="",fechanew="",sumatotal="",cant_sur_vacio="",valor_exis="",indi="",mensaje="",id_send="";
...
if(but.equals("Clave"))
  { // inicio de if but
	...
	pos=0;
	ban_clave=1;
	// se toman los valores de la receta
	date2_jv=request.getParameter("txtf_date");
	date_jv=request.getParameter("txtf_date");
	hora_jv=request.getParameter("reloj");
 ...
 if( exist_0 == 0 )
  {
	  ...
	  while(rset.next())
	  {
		 ...
		 if(resto==0)
		 {
		   ...
		   {cant_x_sutir=resto;
		    ...
		    if(cant_x_sutir<=exis_restantes)
			{
				...
				stmt1.execute("insert into movimientos_salidas values ('"+rset.getString("clave")+"','"+rset.getString("descrip")+"','"+rset.getString("lote")+"','"+rset.getString("caducidad")+"','"+cant_por_surtir+"','1','"+cant_por_surtir+"','"+rset.getString("origen")+"','-','"+df.format(new java.util.Date())+"','"+reloj_jv+"','4')"); 
		...
		// se agrega info a la tabla modificacion
				 cant_x_sutir_neg*=-1;
					  stmt1.execute("insert into modificacion values ('"+rset.getString("clave")+"','"+rset.getString("descrip")+"','"+rset.getString("lote")+"','"+rset.getString("caducidad")+"','"+exis_restantes+"','"+rset.getString("origen")+"','"+cant_x_sutir_neg+"','"+exis_total_num+"','"+df.format(new java.util.Date())+"','"+ela2_jv+"','SALIDA DE INSUMO','SALIDA DE INSUMO EN RECETA COLECTIVA VIA OPCION MODIFICACION',current_timestamp,'"+foliore_jv+"','-','2','-','"+present1_jv+"','"+exis_total_num+"',0)");						 
					 // fin tabla modi 	
		if(resto==0)
		...
		stmt1.execute("insert into receta_colectiva values ('"+foliore_jv+"','"+fechanew+"','"+reloj_jv+"','"+juris_jv+"','"+no_jur+"','"+nom_unidad+"','"+nombre_jv+"','"+edad_jv+"','"+folio_jv+"','"+programa_jv+"','-','"+cedu_jv+"','"+nomed_jv+"','"+cause_jv+"','"+clave1_jv+"','"+descrip1_jv+"','"+cant_por_surtir+"','"+cant_por_surtir+"','"+status_surt+"','"+ela_jv+"','"+clave_jv+"','"+id_med_jv+"','"+cv_dgo_jv+"','"+cv_uni_jv+"','"+cv_finan_jv+"','"+financiamiento1+"','"+layout+"','"+rset.getString("origen")+"','"+present1_jv+"','"+resto_jv+"','"+status+"','RC','-','-','"+rset.getString("lote")+"','"+rset.getString("caducidad")+"','"+carnet_jv+"','"+servicio_jv+"','"+fechanew+"','A','"+encarser_jv+"','"+indi+"','-','-','-','-','-','"+corte+"','"+ampuleo+"',0)");	
			...
			else
			{
				...
				stmt1.execute("insert into movimientos_salidas values ('"+rset.getString("clave")+"','"+rset.getString("descrip")+"','"+rset.getString("lote")+"','"+rset.getString("caducidad")+"','"+exis_restantes+"','1','"+exis_restantes+"','"+rset.getString("origen")+"','-','"+fechanew+"','"+reloj_jv+"','4')"); 
				// fin insert a movimientos_salida
   ...
   while(rset.next())
	  {
		 ...
		 {
		  //se hace un insert a movimientos_salida
				stmt1.execute("insert into movimientos_salidas values ('"+rset.getString("clave")+"','"+rset.getString("descrip")+"','"+rset.getString("lote")+"','"+rset.getString("caducidad")+"','"+exis_may+"','1','"+exis_may+"','"+rset.getString("origen")+"','-','"+df.format(new java.util.Date())+"','"+reloj_jv+"','4')"); 
...
if(but.equals("Imprimir"))
	...
	encarser_jv=request.getParameter("txtf_encarser");
	  ...
	  %>
	  <script>
	  self.location='ticket2_rcmodi2.jsp?folio=<%=foliore2_jv%>&folio2=<%=customerIds%>&receta5=RC&encar=<%=ela2_jv%>&uni=<%=nom_unidad%>&juris=<%=no_jur%>&juris1=<%=juris_jv%>&clave_uni=<%=clave_jv%>&cv_dgo=<%=cv_dgo_jv%>&cv_uni=<%=cv_uni_jv%>&cv_mpio=<%=cv_mpio_jv%>&encarser=<%=encarser_jv%>&ban=<%=tipo_rec%>&usuario=<%=usuario%>';
...
<form name="form" method="post" action="rc_modi2.jsp?usuario=<%=usuario%>&ban=<%=tipo_rec%>">
      ...
      <table width="914" height="61" border="0" align="center" cellpadding="2">
        <tr>
		 ...
		 %>
          ...
          <td width="291" class="style2">FECHA:<input readonly name="txtf_date" id="txtf_date" type="text" class="Estilo5" onKeyPress="return handleEnter(this, event)" value="<%=date_jv%>" size="9"/>
          <img src="imagenes/cal.jpg" width="27" height="26" border="0" onClick="scwShow(scwID('txtf_date'), event)" scr="imagenes/cal.jpg" />&nbsp; HORA:
                <input name="reloj" type="text" value="<%=hora_jv%>"  class="Estilo5" onKeyPress="return handleEnter(this, event)" size="8" >         
            </td>
          <td width="153" align="center" class="style2"> No. Folio
            ...
            <tr>
              <td width="144" height="100%"><label>
                <input name="txtf_foliore" type="text" class="Estilo5" value="<%=foliore_jv%>" size="10" />
              </label></td>
            </tr>
          </table></td>
        </tr>
        ...
        <tr>
          <td height="71" colspan="3" class="style4"><table width="784" border="0" align="center" cellpadding="2">
            ...
            <tr>
              <td colspan="2"><textarea name="txtf_unidadmed" cols="40" class="Estilo5" colspan="3"><%=nom_unidad%></textarea>
		...
		<table width="939" border="0" align="center" >
            <tr>
              ...
              <textarea name="txtf_servicio" cols="30" class="Estilo5"><%=servicio_jv%></textarea>
              </span></td>
    ...
    %>              
            <tr>
			  ...
			  <%
				...
				surtida=Integer.parseInt(sur1_jv);
				clave1_jv=rset.getString("clave");
				part_jv=rset.getString("partida");
				id_send=rset.getString("id");
				
			  %>
              <td class="style2">
              <input  type="button" id="btn_<%=eliminar_jv%>" name="btn_<%=eliminar_jv%>" class="subHeader" value="Eliminar" onclick='location.href="eliminar_col2.jsp?id_prov=<%=eliminar_jv%>&foliore=<%=foliore_jv%>&encar=<%=ela2_jv%>&juris=<%=no_jur%>&uni=<%=nom_unidad%>&juris1=<%=juris_jv%>&clave_uni=<%=clave_jv%>&cv_dgo=<%=cv_dgo_jv%>&cv_uni=<%=cv_uni_jv%>&cv_mpio=<%=cv_mpio_jv%>&day5=<%=t1_jv%>&mes5=<%=t2_jv%>&aa5=<%=t3_jv%>&servicio=<%=servicio_jv%>&encarser=<%=encarser_jv%>&clave1=<%=clave1_jv%>&sol1=<%=sol1_jv%>&sur1=<%=sur1_jv%>&lote=<%=lotes_jv%>&cadu=<%=cadus_jv%>&part=<%=part_jv%>&ban=<%=tipo_rec%>&usuario=<%=usuario%>"'/>
  <%
  if(surtida!=1)
  {
  %>
  
  <input  type="button" id="btn_<%=eliminar_jv%>2" name="btn_<%=eliminar_jv%>2" class="subHeader" value="Modificar" onclick='location.href="modificar_rc.jsp?id_prov=<%=eliminar_jv%>&uni=<%=nom_unidad%>&juris=<%=no_jur%>&foliore=<%=foliore_jv%>&clave1=<%=clave1_jv%>&descrip1=<%=descrip1_jv%>&lote=<%=lotes_jv%>&cadu=<%=cadus_jv%>&sol1=<%=sol1_jv%>&sur1=<%=sur1_jv%>&part=<%=part_jv%>&id=<%=id_send%>&mes5=<%=t2_jv%>&usuario=<%=usuario%>"'/>            
   <%
  }
  %>
    ...
    <tr>
      ...
      <tr>
        <td height="40" colspan="3" class="style4"><!--input name="Submit" type="submit" class="subHeader" value="Eliminar Folio" onClick="return verifica_ACT(document.forms.form);"/-->&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input name="Submit" type="submit" class="subHeader" value="Actualizar Datos" onClick="return verifica_ACT(document.forms.form);"/></td>
  </tr>  
